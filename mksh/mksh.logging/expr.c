 ...
 */
int
evaluate(const char *expr, mksh_ari_t *rval, int error_ok, bool arith, int linehint)
{
	struct tbl v;
	int ret;
	...
	v.type = 0;
	ret = v_evaluate(&v, expr, error_ok, arith, linehint);
	*rval = v.val.i;
	   ...
	   bool arith, int linehint)
{
	...
	ATRSHMLOG_GETTIME(400,starttime);
	...
	ATRSHMLOG_GETTIME(401,endtime);
	{
	  if(expr)
	    {
	      int len = strlen(expr);
		  ATRSHMLOG_WRITE(401, 'I',
			      linehint ? linehint : g_linehint, 
			      starttime,
			      endtime,
			      expr,
	    ...
	    ATRSHMLOG_WRITE(401, 'I',
			      ...
			      starttime,
			      endtime,
			"arithmetik expression empty",
			sizeof("arithmetik expression empty") - 1
			      );
	}
...
intvar(Expr_state *es, struct tbl *vp)
{
	...
	if (setint_v(vq, vp, es->arith) == NULL) {
		...
		es->evaling = vp;
		vp->flag |= EXPRINEVAL;
		v_evaluate(vq, str_val(vp), KSH_UNWIND_ERROR, es->arith, 0);
		vp->flag &= ~EXPRINEVAL;
		es->evaling = NULL;
	}
	return (vq);
