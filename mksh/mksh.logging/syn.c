...
pipeline(int cf)
{
	...
	t = get_command(cf);
	if (t != NULL) {
		t->lineno = atrshmlogline;
		while (token(0) == '|') {
			if ((p = get_command(CONTIN)) == NULL)
				syntaxerr(NULL);
			p->lineno = atrshmlogline;
			if (tl == NULL)
			  {
...
get_command(int cf)
{
	...
	case REDIR:
		...
		t = newtp(TCOM);
		if(t && source)
		  g_linehint = t->lineno = source->line;
...
compile(Source *s, bool skiputf8bom)
{
	...
	ATRSHMLOG_GETTIME(300,starttime);
	...
	ATRSHMLOG_GETTIME(301,endtime);
	{
		  ATRSHMLOG_WRITE(301, 'I',
			      0,
			      starttime,
			      endtime,
			      "compile",
