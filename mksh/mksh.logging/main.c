...
main_init(int argc, const char *argv[], Source **sp, struct block **lp)
{
 ...
 begin_parse_kshname:
	...
	/* atr */
	/* if we have set the atrshmlog_shmid we need to import it too
	 * to the internal variables 
	 */
		
	if (ATRSHMLOG_GET_SHMID() != 0)
	  {
	    char buffer[256];
	    strcpy(buffer, ATRSHMLOG_GET_ENVPREFIX());
	    strcat(buffer, ATRSHMLOG_GETENVIDSUFFIX());
	    vp = global( buffer );
	    setint_n(vp, ATRSHMLOG_GET_SHMID(), 10);

	    typeset( buffer , IMPORT | EXPORT, 0, 0, 0);
	...
	ATRSHMLOG_GETTIME(4,endtime);
	{
	  if(kshname)
	    {
	      int len =       strlen(kshname);

	      if (argv[0])
	      ATRSHMLOG_WRITE2(4, 'I',
			      1000000,
			      starttime,
			      endtime,
			      kshname,
	...
	if (Flag(FLOGIN)) {
	  include(MKSH_SYSTEM_PROFILE, 0, NULL, true, 0);

		ATRSHMLOG_GETTIME(5,endtime);
		{
		  if(MKSH_SYSTEM_PROFILE)
		    {
		  int len =   strlen(MKSH_SYSTEM_PROFILE);
		  ATRSHMLOG_WRITE(5, 'I',
			      1000000,
			      starttime,
			      endtime,
		  ...
		  ATRSHMLOG_WRITE(5, 'I',
			      ...
			      starttime,
			      endtime,
			      "MKSH_SYSTEM_PROFILE empty",
			      sizeof("MKSH_SYSTEM_PROFILE empty") - 1
	...
	if (!Flag(FPRIVILEGED)) {
	  if (Flag(FLOGIN)) {
	    include(substitute("$HOME/.profile", 0, 0), 0, NULL, true,0);
		ATRSHMLOG_GETTIME(6,endtime);
		{
		  ATRSHMLOG_WRITE(6, 'I',
			      1000000,
			      starttime,
	  ...
	  include(MKSH_SUID_PROFILE, 0, NULL, true,0);
		/* turn off -p if not set explicitly */
		if (Flag(FPRIVILEGED) != 1)
			change_flag(FPRIVILEGED, OF_INTERNAL, false);
		ATRSHMLOG_GETTIME(7,endtime);
		{
		  if (MKSH_SUID_PROFILE)
		    {
		  int len =   strlen(MKSH_SUID_PROFILE);
		  ATRSHMLOG_WRITE(7, 'I',
			      1000000,
			      starttime,
			      endtime,
		  ...
		  ATRSHMLOG_WRITE(7, 'I',
			      ...
			      starttime,
			      endtime,
			      "MKSH_SUID_PROFILE empty",
			      sizeof("MKSH_SUID_PROFILE empty") - 1 
	...
	*lp = l;
	...
	ATRSHMLOG_GETTIME(9,endtime);
	{
	  static const char buffer[] = "after init";
...
main(int argc, const char *argv[])
{
	...
	{
	  static const char buffer[] = "hallo welt";
	  
	  ATRSHMLOG_WRITE(1, 'I',
		       9000000 + ATRSHMLOG_GET_VERSION(),
		       starttime,
		       endtime,
		       buffer,
		       sizeof(buffer) - 1
	...
	ATRSHMLOG_GETTIME(1,lastendtime);
	{
	  static const char buffer[] = "hallo welt";
	  
	  ATRSHMLOG_WRITE(1, 'I',
		       8000000 + ATRSHMLOG_GET_VERSION(),
		       endtime,
		       lastendtime,
		       buffer,
		       sizeof(buffer) - 1
	...
	for (int j = 0; j < 32; ++j)
	  {
	    memcpy(&lastendtime, &endtime, sizeof(lastendtime));
	    ATRSHMLOG_GETTIME(1,endtime);
	    {
	      static const char buffer[] = "hallo welt too";
	  
	      ATRSHMLOG_WRITE(1, 'I',
			   1000000 + ATRSHMLOG_GET_VERSION(),
			   lastendtime,
			   endtime,
			   buffer,
			   sizeof(buffer) - 1
	...
	/* atr */
	ATRSHMLOG_GETTIME(2,endtime);
	{
	  static const char buffer[] = "hallo welt too second last";
	  
	  ATRSHMLOG_WRITE(2, 'I',
		       2000000 + ATRSHMLOG_GET_VERSION(),
		       lastendtime,
		       endtime,
		       buffer,
		       sizeof(buffer) - 1
	...
	/* atr */
	ATRSHMLOG_GETTIME(2,lastendtime);
	ATRSHMLOG_GETTIME(2,endtime);
	ATRSHMLOG_GETTIME(2,endtime);
	ATRSHMLOG_GETTIME(2,endtime);
	ATRSHMLOG_GETTIME(2,endtime);
	...
	ATRSHMLOG_GETTIME(2,endtime);
	{
	  static const char buffer[] = "hallo welt too last";
	  
	  ATRSHMLOG_WRITE(2, 'I',
		       3000000 + ATRSHMLOG_GET_VERSION(),
		       lastendtime,
		       endtime,
		      "hallo welt 10 times gettime",
		       sizeof("hallo welt 10 times gettime") - 1
		       );

	  ATRSHMLOG_WRITE(2, 'I',
		       4000000 + ATRSHMLOG_GET_VERSION(),
		       lastendtime,
		       endtime,
		       buffer,
		       sizeof(buffer) - 1
	...
	ATRSHMLOG_GETTIME(2,lastendtime);
	  ATRSHMLOG_WRITE(2, 'I',
		       5000000 + ATRSHMLOG_GET_VERSION(),
		       endtime,
		       lastendtime,
		       "hallo welt 10 times write",
		       sizeof("hallo welt 10 times write") - 1
		       );
	}
	/* atr */
	...
	if ((rv = main_init(argc, argv, &s, &l)) == 0) {
		if (Flag(FAS_BUILTIN)) {
	ATRSHMLOG_GETTIME(10,endtime);
	{
	  static const char buffer[] = "after init before comexec";
	  
	  ATRSHMLOG_WRITE(10, 'I',
		       2000000,
		       starttime,
		       endtime,
		       buffer,
	...
	ATRSHMLOG_GETTIME(10,endtime);
	{
	  static const char buffer[] = "after init before shell";
	...
	ATRSHMLOG_GETTIME(11,endtime);
	{
	  static const char buffer[] = "end of sh_main";
	  
	  ATRSHMLOG_WRITE(11, 'I',
		       2000000,
		       starttime,
		       endtime,
		       buffer,
	...
	int linehint)
{
	...
	ATRSHMLOG_GETTIME(20,starttime);

	shf = shf_open(name, O_RDONLY, 0, SHF_MAPHI | SHF_CLEXEC);
	if (shf == NULL) {

	  ATRSHMLOG_GETTIME(21,endtime);

	if (name)
	    {
	      int len = 		      strlen(name);
	  ATRSHMLOG_WRITE(21, 'I',
		      linehint,
		      starttime,
		      endtime,
		      name,
	...
	if ((i = kshsetjmp(e->jbuf))) {
		...
		case LERROR:
			/* see below */
	ATRSHMLOG_GETTIME(22,endtime);
	{
	  if(name)
	    {
	      int len = strlen(name);
	  ATRSHMLOG_WRITE(22, 'I',
		      linehint,
		      starttime,
		      endtime,
		      name,
	...
	ATRSHMLOG_GETTIME(23,endtime);
	{

	  if(name)
	    {
	      int len = 	      strlen(name);
	  ATRSHMLOG_WRITE(23, 'I',
		      linehint,
		      starttime,
		      endtime,
		      name,
	...
	ATRSHMLOG_GETTIME(24,endtime);
	{

	  if(name)
	    {
	      int len = 	      strlen(name);
	  ATRSHMLOG_WRITE(24, 'I',
		      linehint, /* dot file, filename */
		      starttime,
		      endtime,
		      name,
			      len < ATRSHMLOGENTRYSIZELIMIT ? len : ATRSHMLOGENTRYSIZELIMIT
...
command(const char *comm, int line)
{
	...
	ATRSHMLOG_GETTIME(30,starttime);
	...
	ATRSHMLOG_GETTIME(31,endtime);
	{
	  if(comm)
	    {
	      int len = strlen(comm);
	  ATRSHMLOG_WRITE(31, 'I',
		      line,
		      starttime,
		      endtime,
		      comm,
...
shell(Source * volatile s, volatile bool toplevel)
{
	...
	ATRSHMLOG_GETTIME(40,starttime);
	...
	case LRETURN:
		source = old_source;
		quitenv(NULL);
				ATRSHMLOG_GETTIME(41,endtime);
				{
				  if(s && s->file)
				    {
				      int len = strlen(s->file);
				  ATRSHMLOG_WRITE(41, 'I',
					       s->line,
					       starttime,
					       endtime,
					       s->file,
	...
	default:
		source = old_source;
		quitenv(NULL);
				ATRSHMLOG_GETTIME(42,endtime);
				{
				  if(s && s->file)
				    {
				      int len = strlen(s->file);
				  ATRSHMLOG_WRITE(42, 'I',
					       s->line,
					       starttime,
					       endtime,
					       s->file,
 ...
 source_no_tree:
	...
	ATRSHMLOG_GETTIME(43,endtime);
	{
				  if(s && s->file)
				    {
				      int len = strlen(s->file);
				  ATRSHMLOG_WRITE(43, 'I',
					      s->line, /* dot file, file name */
					       starttime,
					       endtime,
					       s->file,
					       		      len < ATRSHMLOGENTRYSIZELIMIT ? len : ATRSHMLOGENTRYSIZELIMIT
