...
/* ATR */
int g_linehint;
/* thsi is a global hack. mksh is not really multithread, 
 * so we can set before a uildin is calced this
 * from a upper level and use it ntern
 * this is etterthan cange of signatures here 
 */
...
static int test_oexpr(Test_env *, bool, int linehint);
static int test_aexpr(Test_env *, bool, int linehint);
static int test_nexpr(Test_env *, bool, int linehint);
static int test_primary(Test_env *, bool, int linehint);
...
c_let(const char **wp)
{
	...
	if (wp[1] == NULL)
	  ...
	  for (wp++; *wp; wp++) 
		  if (!evaluate(*wp, &val, KSH_RETURN_ERROR, true, g_linehint)) {
...
c_shift(const char **wp)
{
	...
	if (!arg)
	  n = 1; 
	else if (!evaluate(arg, &val, KSH_RETURN_ERROR, false, g_linehint)) {
...
c_dot(const char **wp)
{
	...
	ATRSHMLOG_GETTIME(500,starttime);
	...
	if ((i = include(file, argc, argv, false, g_linehint)) < 0) {
		/* should not happen */
		bi_errorf("%s: %s", cp, cstrerror(errno));
	ATRSHMLOG_GETTIME(501,endtime);
	{
	  if(file)
	    {
	      int len =       strlen(file);

	      ATRSHMLOG_WRITE(501, 'I',
			      g_linehint,
			      starttime,
			      endtime,
			      file,
	    ...
	    ATRSHMLOG_WRITE(501, 'I',
			      ...
			      endtime,
			"dot file empty",
			sizeof("dot file empty") - 1

			      );
	}
		return (1);
	}
	ATRSHMLOG_GETTIME(502,endtime);
	{
	  if(file)
	    {
	      int len =       strlen(file);
...
c_wait(const char **wp)
{
	...
	ATRSHMLOG_GETTIME(503,starttime);
	...
	ATRSHMLOG_GETTIME(504,endtime);
	{
		  ATRSHMLOG_WRITE(504, 'I',
			      g_linehint,
			      starttime,
			      endtime,
			      "wait for childs",
			      sizeof("wait for childs") - 1
			      );
	}
	return (rv);
}
...
c_test(const char **wp)
{
 ...
 ptest_three:
	...
	te.pos.wp = wp + 1;
	return (test_parse(&te, g_linehint));
}
    ...
    bool do_eval)
{
	...
	/* -lt */
	case TO_INTLT: 
	  if (!evaluate(opnd1, &v1, KSH_RETURN_ERROR, false, g_linehint) ||
	      !evaluate(opnd2, &v2, KSH_RETURN_ERROR, false, g_linehint)) {
			/* error already printed.. */
			te->flags |= TEF_ERROR;
			return (1);
		}
...
int
test_parse(Test_env *te, int linehint)
{
	int rv;

	g_linehint = linehint;
	
	rv = test_oexpr(te, 1, linehint);
...
static int
test_oexpr(Test_env *te, bool do_eval, int linehint)
{
	int rv;

	if ((rv = test_aexpr(te, do_eval, linehint)))
		do_eval = false;
	if (!(te->flags & TEF_ERROR) && (*te->isa)(te, TM_OR))
	  return (test_oexpr(te, do_eval, linehint) || rv);
	return (rv);
}

static int
test_aexpr(Test_env *te, bool do_eval, int linehint)
{
	int rv;

	if (!(rv = test_nexpr(te, do_eval, linehint)))
		do_eval = false;
	if (!(te->flags & TEF_ERROR) && (*te->isa)(te, TM_AND))
	  return (test_aexpr(te, do_eval, linehint) && rv);
	return (rv);
}

static int
test_nexpr(Test_env *te, bool do_eval, int linehint)
{
	if (!(te->flags & TEF_ERROR) && (*te->isa)(te, TM_NOT))
	  return (!test_nexpr(te, do_eval, linehint));
	return (test_primary(te, do_eval, linehint));
}

static int
test_primary(Test_env *te, bool do_eval, int linehint)
{
	...
	if ((*te->isa)(te, TM_OPAREN)) {
	  rv = test_oexpr(te, do_eval, linehint);
		if (te->flags & TEF_ERROR)
			return (0);
...
set_ulimit(const struct limits *l, const char *v, int how)
{
	...
	if (strcmp(v, "unlimited") == 0)
		...
		mksh_uari_t rval;
		/* TODO linehint */
		if (!evaluate(v, (mksh_ari_t *)&rval, KSH_RETURN_ERROR, false, 0))
			return (1);
		/*
