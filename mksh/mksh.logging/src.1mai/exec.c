...
static int call_builtin(struct tbl *, const char **, const char *, bool, int linehint);
//static int iosetup(struct ioword *, struct tbl *);
static int iosetup(struct ioword *, struct tbl *, int linehint);
    ...
    volatile int * volatile xerrok)
{
	...
	ATRSHMLOGGETTIME(200,starttime);

	atrshmlogline = t->lineno;

	if(atrshmlogline == 0)
	  atrshmlogline = g_linehint;
	else
	  g_linehint = atrshmlogline;
	...
	if (t->type == TCOM) {
		 ...
		 */
		up = eval(t->args, t->u.evalflags | DOBLANK | DOGLOB | DOTILDE, atrshmlogline);
		if (flags & XTIME)
	...
	if (t->ioact != NULL)
		for (iowp = t->ioact; *iowp != NULL; iowp++) {
		  if (iosetup(*iowp, tp, atrshmlogline) < 0) {
				exstat = rv = 1;
				/*
	...
	case TCOM:
		...
		ATRSHMLOGGETTIME(204,endtime);
		{
		  if (ap[0])
		    {
		      int len = strlen(ap[0]);
		      
		  ATRSHMLOGWRITE(204, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
			      ap[0],
	...
	case TPAREN:
		...
		ATRSHMLOGGETTIME(205,endtime);
		{
		    ATRSHMLOGWRITE(205, 'I',
			       atrshmlogline,
			      starttime,
			      endtime,
				"(", /* ) */
	...
	case TPIPE:
		...
		while (t->type == TPIPE) {
			 ...
			 * will hang forever).
			 */
			if (atrshmlogline == 0 && t->left)
			  g_linehint = atrshmlogline = t->left->lineno;
		...
		if (!(flags&XBGND) && !(flags&XXCOM))
			rv = i;
		if (atrshmlogline == 0 && t)
			  g_linehint = atrshmlogline = t->lineno;

		
		ATRSHMLOGGETTIME(206,endtime);
		{
		    ATRSHMLOGWRITE(206, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"pipe", /* ) */
	...
	case TLIST:
		...
		if (t && atrshmlogline == 0)
		  atrshmlogline = t->lineno ;
		
		ATRSHMLOGGETTIME(207,endtime);
		{
		    ATRSHMLOGWRITE(207, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"list", /* ) */
	...
	case TCOPROC: {
		...
		ATRSHMLOGGETTIME(208,endtime);
		{
		  if(t->left)
		    ATRSHMLOGWRITE(208, 'I',
				t->left->lineno, 
			      starttime,
			      endtime,
				"co process", /* ) */
	...
	case TASYNC:
		...
		ATRSHMLOGGETTIME(209,endtime);
		{
		  if(t->left)
		    ATRSHMLOGWRITE(209, 'I',
			      	t->left->lineno,
			      starttime,
			      endtime,
				"async process", /* ) */
				sizeof("async process") - 1 /* ) */
				);
		  else
		    ATRSHMLOGWRITE(209, 'I',
				t ? t->lineno : atrshmlogline,
	...
	case TAND:
		...
		ATRSHMLOGGETTIME(210,endtime);
		{
		    ATRSHMLOGWRITE(210, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				t->type == TAND ? "AND" : "OR ",
	...
	case TBANG:
		...
		if (xerrok)
			*xerrok = 1;
		ATRSHMLOGGETTIME(211,endtime);
		{
		    ATRSHMLOGWRITE(211, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"!", /* ) */
	...
	case TDBRACKET: {
		...
		rv = test_parse(&te, atrshmlogline);

		ATRSHMLOGGETTIME(212,endtime);
		{
		  if(t->args[0])
		    {
		    ATRSHMLOGWRITE2(212, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"[[", /* ) */
	...
	case TSELECT: {
		...
		ap = (t->vars == NULL) ? e->loc->argv + 1 :
		    (const char **)eval((const char **)t->vars,
					DOBLANK | DOGLOB | DOTILDE, atrshmlogline);
		e->type = E_LOOP;
		...
		ATRSHMLOGGETTIME(213,endtime);
		{
		  if(t->str)
		    {
		      const char* largv[] =  { t->str, NULL};
		      
		      ATRSHMLOGWRITE2(213, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				   "for / select",
		  ...
		  ATRSHMLOGWRITE(213, 'I',
			      t ? t->lineno : atrshmlogline,
			      starttime,
			      endtime,
			      "for /select empty",
			      sizeof("for /select empty") -1
				);
		}
		break;
	}
	...
	case TUNTIL:
		...
		while ((i = kshsetjmp(e->jbuf))) {
			if ((e->flags&EF_BRKCONT_PASS) ||
			    (i != LBREAK && i != LCONTIN)) {
				quitenv(NULL);
		ATRSHMLOGGETTIME(216,endtime);
		{
		    ATRSHMLOGWRITE(216, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
				"while / until", /* ) */
				sizeof("while / until") - 1 /* ) */
				);
		}
				unwind(i);
			} else if (i == LBREAK) {
				rv = 0;
		ATRSHMLOGGETTIME(215,endtime);
		{
		    ATRSHMLOGWRITE(215, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"while / until break", /* ) */
				sizeof("while / until break") - 1 /* ) */
				);
		}
				goto Break;
			}
		...
		ATRSHMLOGGETTIME(214,endtime);
		{
		    ATRSHMLOGWRITE(214, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"while / until", /* ) */
	...
	case TELIF:
		...
		ATRSHMLOGGETTIME(218,endtime);
		{
		    ATRSHMLOGWRITE(218, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"if / elif", /* ) */
	...
	case TCASE:
		...
		ccp = evalstr(t->str, DOTILDE | DOSCALAR, atrshmlogline);

		for (t = t->left; t != NULL && t->type == TPAT; t = t->right) {
			for (ap = (const char **)t->vars; *ap; ap++) {
			  if (i || ((s = evalstr(*ap, DOTILDE|DOPAT, atrshmlogline)) &&
				    gmatchx(ccp, s, false))) {
				  if(t->left)
 ...
 TCASE_out:
		ATRSHMLOGGETTIME(219,endtime);
		{
		  if (p)
		    {
		      const char *largv[] = { p, NULL};
		      ATRSHMLOGWRITE2(219, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"case" ,
	...
	case TBRACE:
		rv = execute(t->left, flags & XERROK, xerrok);
		ATRSHMLOGGETTIME(220,endtime);
		{
		    ATRSHMLOGWRITE(220, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"{", /* ) */
	...
	case TFUNCT:
		rv = define(t->str, t);
		ATRSHMLOGGETTIME(221,endtime);
		{
		  if(t->str)
		    {

		      const char* largv[] = {t->str, NULL};
		    ATRSHMLOGWRITE2(221, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
				 "define function",
				 sizeof("define function") - 1 ,
				    largv,
				    -1
				);
		    }
		  else
		    ATRSHMLOGWRITE(221, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
				"function empty",
				sizeof("function empty") - 1
	...
	case TTIME:
		...
		ATRSHMLOGGETTIME(222,endtime);
		{
		    ATRSHMLOGWRITE(222, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"time", /* ) */
	...
	case TEXEC:
		...
		cleanup_proc_env();
		{
			...
			cargs.ro = t->args;
			/* atr : no more atrshmlog here. 
			 * we are here in a subshell
			 * and we have only an execute before the execve ... 
			 * thats redundant to events 278 and 204 and 202
			 */
    ...
    volatile int flags, volatile int *xerrok)
{
	...
	ATRSHMLOGGETTIME(270,starttime);
	...
	for (i = 0; t->vars[i]; i++) {
		...
		e->loc = l_expand;
		cp = evalstr(t->vars[i], DOASNTILDE | DOSCALAR, atrshmlogline);
		e->loc = l_assign;
		if (Flag(FXTRACE)) {
			const char *ccp;
 ...
 xtrace_ap_loop:
	...
	case CSHELL:
 do_call_builtin:
	  rv = call_builtin(tp, (const char **)ap, null, resetspec, atrshmlogline);
	...
	case CFUNC: {
		...
		const char * volatile atrshmlogname = NULL;
		...
		if (!(tp->flag & ISSET)) {
			...
			if (include(tp->u.fpath, 0, NULL, false, atrshmlogline) < 0) {
				if (!strcmp(cp, Tcat)) {
 no_cat_in_FPATH:
					tp = findcom(Tcat, FC_BI);
		ATRSHMLOGGETTIME(275,endtime);
		{
		  if (cp)
		    {
		  int len =	strlen(cp);
		    ATRSHMLOGWRITE(275, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				cp,
 ...
 no_printf_in_FPATH:
		...
		ATRSHMLOGGETTIME(276,endtime);
		{
		  if (cp)
		    {
		  int len = 	strlen(cp);
		    ATRSHMLOGWRITE(276, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				cp,
		...
		ATRSHMLOGGETTIME(274,endtime);
		{
		  if(atrshmlogname)
		    {
		      int len = strlen(atrshmlogname);
		    ATRSHMLOGWRITE2(274, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				atrshmlogname,
	...
	case CTALIAS:
		...
		ATRSHMLOGGETTIME(277,endtime);
		{
		    ATRSHMLOGWRITE(277, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"alias exec error",
				sizeof("alias exec error") - 1
				);
		}
			break;
		}
		...
		texec.str = tp->val.s;
		texec.args = ap;
		texec.lineno = atrshmlogline;
		  
		{
		  if(tp->val.s)
		    {
		  int len = strlen(tp->val.s);
		    ATRSHMLOGWRITE2(279, 'P',
				atrshmlogline,
			      starttime,
			      starttime,
				tp->val.s,
		...
		ATRSHMLOGGETTIME(278,endtime);
		{
		  if(tp->val.s)
		    {
		  int len = strlen(tp->val.s);
		    ATRSHMLOGWRITE(278, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				tp->val.s,
...
scriptexec(struct op *tp, const char **ap)
{
	...
	const char * volatile atrshmlogname = NULL;
	/* we assume as below tp is not null */
	int atrshmlogline = 0;

	if (tp)
	  atrshmlogline = tp->lineno;
	...
	ATRSHMLOGGETTIME(250,starttime);
 ...
 nomagic:
	...
	*args.ro = sh;
	...
	ATRSHMLOGGETTIME(251,endtime);
		{
		  if (atrshmlogname)
		    {
		  int len = 	strlen(atrshmlogname);
		    ATRSHMLOGWRITE(251, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				atrshmlogname,
...
shcomexec(const char **wp)
{
	...
	return (call_builtin(tp, wp, "shcomexec", false, 0));
}
...
define(const char *name, struct op *t)
{
	...
	ATRSHMLOGGETTIME(290,starttime);
	...
	if (t == NULL) {
		/* undefine */
		ktdelete(tp);
		ATRSHMLOGGETTIME(291,endtime);
		{
		  if (name)
		    {
		  const char *largv[] = {name, NULL};
		  
		    ATRSHMLOGWRITE2(291, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				 "undefine",
		    ...
		    ATRSHMLOGWRITE(291, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"undefine empty",
				sizeof("undefine empty") - 1
	...
	if (t->u.ksh_func)
		...
		ATRSHMLOGGETTIME(292,endtime);
		{
		  if (name)
		    {

		      const char* largv[] = { name, NULL };
		    ATRSHMLOGWRITE2(292, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				 "define function",
		    ...
		    ATRSHMLOGWRITE(292, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"define empty",
				sizeof("define empty") - 1 
	     ...
	     int linehint)
{
	...
	ATRSHMLOGGETTIME(280,starttime);
	...
	ATRSHMLOGGETTIME(281,endtime);
		{
		  if (wp[0] && wp[1])
		    {
		      int len = strlen(wp[0]);
		      ATRSHMLOGWRITE2(281, 'I',
			      linehint,
			      starttime,
			      endtime,
				wp[0],
 ...
 */
static int
iosetup(struct ioword *iop, struct tbl *tp, int linehint)
{
	int u = -1;
	char *cp = iop->ioname;
	...
	ATRSHMLOGGETTIME(260,starttime);

	if (iotype != IOHERE)
	  cp = evalonestr(cp, DOTILDE|(Flag(FTALKING_I) ? DOGLOB : 0), linehint);
	...
	case IODUP: {
		...
		if (ksh_isdash(cp)) {
			...
			ATRSHMLOGGETTIME(261,endtime);
			{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOGWRITE(261, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	  ...
	  ATRSHMLOGWRITE(261, 'I',
		...
		if (u == (int)iop->unit) {
			/* "dup from" == "dup to" */
		  ATRSHMLOGGETTIME(268,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOGWRITE(268, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
 ...
 clobber_refused:
	...
	if (u < 0) {
		...
		ATRSHMLOGGETTIME(262,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOGWRITE(262, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	...
	if (do_close)
		...
		if (ksh_dup2(u, iop->unit, true) < 0) {
			...
			if (iotype != IODUP)
				close(u);
		ATRSHMLOGGETTIME(264,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = strlen(iotmp.ioname);
	  ATRSHMLOGWRITE(264, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	...
	ATRSHMLOGGETTIME(269,endtime);
	{
	  if (iotmp.ioname)
	    {
	      int len  = strlen(iotmp.ioname);
	      char buff[32];
	      sprintf(buff, "%x", iotype);

	      const char* largv[2] = {buff, NULL };
	      
	  ATRSHMLOGWRITE2(269, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	  ...
	  ATRSHMLOGWRITE(269, 'I',
		      linehint,
			      starttime,
			      endtime,
		      "iosetup end",
		      sizeof("iosetup end") -1
				);
		}
...
hereinval(struct ioword *iop, int sub, char **resbuf, struct shf *shf)
{
	...
	if (iop->ioflag & IOHERESTR) {
	  ccp = evalstr(iop->delim, DOHERESTR | DOSCALAR | DOHEREDOC, 0);
	} else if (sub) {
		...
		source = osource;
		ccp = evalstr(yylval.cp, DOSCALAR | DOHEREDOC, 0);
	}
...
dbteste_getopnd(Test_env *te, Test_op op, bool do_eval)
{
	...
	return (evalstr(s, flags, g_linehint));
}
