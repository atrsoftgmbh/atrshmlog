...
static int comsub(Expand *, const char *, int, int linehint);
...
char *
substitute(const char *cp, int f, int linehint)
{
	...
	ATRSHMLOG_GETTIME(100,starttime);
	
	{
	  if(cp)
	    {
	      int len = strlen(cp);
	  ATRSHMLOG_WRITE(101, 'P',
		      linehint,
		      starttime,
		      starttime,
		      cp,
	...
	char* res = evalstr(yylval.cp, f, linehint);
	ATRSHMLOG_GETTIME(102,endtime);
	{

	  if(res)
	    {
	      int len = strlen(res);
	  ATRSHMLOG_WRITE(102, 'I',
		      linehint,
		      starttime,
		      endtime,
		      res,
 ...
 */
char **
eval(const char **ap, int f, int linehint)
{
	XPtrV w;
	...
	XPput(w, NULL);
	while (*ap != NULL)
	  expand(*ap++, &w, f, linehint);
	XPput(w, NULL);
	char** res = (char **)XPclose(w) + 1;
 ...
 */
char *
evalstr(const char *cp, int f, int linehint)
{
	XPtrV w;
	char *dp = null;
	...
	ATRSHMLOG_GETTIME(120,starttime);
	
	XPinit(w, 1);
	expand(cp, &w, f, linehint);
	if (XPsize(w))
		dp = *XPptrv(w);
	XPfree(w);


	ATRSHMLOG_GETTIME(121,endtime);
	if (dp)
	  {
	    int len = strlen(dp);
	    ATRSHMLOG_WRITE(123, 'I',
			linehint, 
			starttime,
			endtime,
			dp,
 ...
 */
char *
evalonestr(const char *cp, int f, int linehint)
{
	XPtrV w;
	char *rv;
	...
	case 1:
		...
		break;
	default:
	  rv = evalstr(cp, f&~DOGLOB, linehint);
		break;
	}
	XPfree(w);
	return (rv);
       ...
       int linehint)
{
	...
	ATRSHMLOG_GETTIME(130,starttime);
			...
			case EXPRSUB:
				...
				if (f & DONTRUNCOMMAND) {
					...
					*dp++ = ')'; *dp++ = ')';
					...
					v.type = 10;
					v.name[0] = '\0';
					v_evaluate(&v, substitute(sp, 0, linehint),
						   KSH_UNWIND_ERROR, true, linehint);
					sp = strnul(sp) + 1;
					x.str = str_val(&v);
					type = XSUB;
					if (f & DOBLANK)
 ...
 unwind_substsyn:
				...
				if (type == XBASE) {
					...
					case '0': {
						...
						evaluate(substitute(stg = wdstrip(beg, 0), 0, linehint),
							 &from, KSH_UNWIND_ERROR, true, linehint);
						afree(stg, ATEMP);
						if (end) {
						  evaluate(substitute(stg = wdstrip(mid, 0), 0, linehint),
							   &num, KSH_UNWIND_ERROR, true, linehint);
							afree(stg, ATEMP);
						}
						afree(beg, ATEMP);
						beg = str_val(st->var);
					...
					case '/': {
						...
						pat = evalstr(s + (fpat ? 2 : 0),
							      DOTILDE | DOSCALAR | DOPAT, linehint);
						rrep = d ? evalstr(p,
								   DOTILDE | DOSCALAR, linehint) : null;
						afree(s, ATEMP);
 ...
 emit_word:
	...
	ATRSHMLOG_GETTIME(131,endtime);
	{
	  if(ccp)
	    {
	      int len = strlen(ccp);
	  ATRSHMLOG_WRITE(131, 'I',
		      0,
		      starttime,
		      endtime,
		      ccp,
 ...
 */
static int
comsub(Expand *xp, const char *cp, int fn MKSH_A_UNUSED, int linehint)
{
	Source *s, *sold;
	struct op *t;
	struct shf *shf;
	...
	if (t->type == TCOM &&
	    *t->args == NULL && *t->vars == NULL && t->ioact != NULL) {
		...
		shf = shf_open(name = evalstr(io->ioname, DOTILDE, linehint), O_RDONLY,
