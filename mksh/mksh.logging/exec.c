    ...
    volatile int * volatile xerrok)
{
	...
	ATRSHMLOG_GETTIME(200,starttime);
	...
	case TPIPE:
		...
		ATRSHMLOG_GETTIME(206,endtime);
		{
		    ATRSHMLOG_WRITE(206, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"pipe", /* ) */
	...
	case TLIST:
		...
		ATRSHMLOG_GETTIME(207,endtime);
		{
		    ATRSHMLOG_WRITE(207, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"list", /* ) */
	...
	case TCOPROC: {
		...
		ATRSHMLOG_GETTIME(208,endtime);
		{
		  if(t->left)
		    ATRSHMLOG_WRITE(208, 'I',
				t->left->lineno, 
			      starttime,
			      endtime,
				"co process", /* ) */
	...
	case TASYNC:
		...
		ATRSHMLOG_GETTIME(209,endtime);
		{
		  if(t->left)
		    ATRSHMLOG_WRITE(209, 'I',
			      	t->left->lineno,
			      starttime,
			      endtime,
				"async process", /* ) */
	...
	case TAND:
		...
		ATRSHMLOG_GETTIME(210,endtime);
		{
		    ATRSHMLOG_WRITE(210, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				t->type == TAND ? "AND" : "OR ",
	...
	case TBANG:
		...
		if (xerrok)
			*xerrok = 1;
		ATRSHMLOG_GETTIME(211,endtime);
		{
		    ATRSHMLOG_WRITE(211, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"!", /* ) */
	...
	case TDBRACKET: {
		...
		ATRSHMLOG_GETTIME(212,endtime);
		{
		  if(t->args[0])
		    {
		    ATRSHMLOG_WRITE2(212, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"[[", /* ) */
	...
	case TSELECT: {
		...
		ATRSHMLOG_GETTIME(213,endtime);
		{
		  if(t->str)
		    {
		      const char* largv[] =  { t->str, NULL};
		      
		      ATRSHMLOG_WRITE2(213, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				   "for / select",
	...
	case TUNTIL:
		...
		while ((i = kshsetjmp(e->jbuf))) {
			if ((e->flags&EF_BRKCONT_PASS) ||
			    (i != LBREAK && i != LCONTIN)) {
				quitenv(NULL);
		ATRSHMLOG_GETTIME(216,endtime);
		{
		    ATRSHMLOG_WRITE(216, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
				"while / until", /* ) */
				...
				rv = 0;
		ATRSHMLOG_GETTIME(215,endtime);
		{
		    ATRSHMLOG_WRITE(215, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"while / until break", /* ) */
		...
		ATRSHMLOG_GETTIME(214,endtime);
		{
		    ATRSHMLOG_WRITE(214, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"while / until", /* ) */
	...
	case TELIF:
		...
		ATRSHMLOG_GETTIME(218,endtime);
		{
		    ATRSHMLOG_WRITE(218, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"if / elif", /* ) */
 ...
 TCASE_out:
		ATRSHMLOG_GETTIME(219,endtime);
		{
		  if (p)
		    {
		      const char *largv[] = { p, NULL};
		      ATRSHMLOG_WRITE2(219, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"case" ,
	...
	case TBRACE:
		rv = execute(t->left, flags & XERROK, xerrok);
		ATRSHMLOG_GETTIME(220,endtime);
		{
		    ATRSHMLOG_WRITE(220, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"{", /* ) */
	...
	case TFUNCT:
		rv = define(t->str, t);
		ATRSHMLOG_GETTIME(221,endtime);
		{
		  if(t->str)
		    {

		      const char* largv[] = {t->str, NULL};
		    ATRSHMLOG_WRITE2(221, 'I',
			      atrshmlogline,
			      starttime,
			      endtime,
				 "define function",
	...
	case TTIME:
		...
		ATRSHMLOG_GETTIME(222,endtime);
		{
		    ATRSHMLOG_WRITE(222, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"time", /* ) */
    ...
    volatile int flags, volatile int *xerrok)
{
	...
	ATRSHMLOG_GETTIME(270,starttime);
 ...
 no_cat_in_FPATH:
		...
		ATRSHMLOG_GETTIME(275,endtime);
		{
		  if (cp)
		    {
		  int len =	strlen(cp);
		    ATRSHMLOG_WRITE(275, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				cp,
 ...
 no_printf_in_FPATH:
		...
		ATRSHMLOG_GETTIME(276,endtime);
		{
		  if (cp)
		    {
		  int len = 	strlen(cp);
		    ATRSHMLOG_WRITE(276, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				cp,
		...
		ATRSHMLOG_GETTIME(274,endtime);
		{
		  if(atrshmlogname)
		    {
		      int len = strlen(atrshmlogname);
		    ATRSHMLOG_WRITE2(274, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				atrshmlogname,
	...
	case CTALIAS:
		...
		ATRSHMLOG_GETTIME(277,endtime);
		{
		    ATRSHMLOG_WRITE(277, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				"alias exec error",
		...
		ATRSHMLOG_GETTIME(278,endtime);
		{
		  if(tp->val.s)
		    {
		  int len = strlen(tp->val.s);
		    ATRSHMLOG_WRITE(278, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				tp->val.s,
...
scriptexec(struct op *tp, const char **ap)
{
	...
	ATRSHMLOG_GETTIME(250,starttime);
 ...
 nomagic:
	...
	*args.ro = sh;
	...
	ATRSHMLOG_GETTIME(251,endtime);
		{
		  if (atrshmlogname)
		    {
		  int len = 	strlen(atrshmlogname);
		    ATRSHMLOG_WRITE(251, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				atrshmlogname,
...
define(const char *name, struct op *t)
{
	...
	ATRSHMLOG_GETTIME(290,starttime);
	...
	if (t == NULL) {
		/* undefine */
		ktdelete(tp);
		ATRSHMLOG_GETTIME(291,endtime);
		{
		  if (name)
		    {
		  const char *largv[] = {name, NULL};
		  
		    ATRSHMLOG_WRITE2(291, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				 "undefine",
	...
	if (t->u.ksh_func)
		...
		ATRSHMLOG_GETTIME(292,endtime);
		{
		  if (name)
		    {

		      const char* largv[] = { name, NULL };
		    ATRSHMLOG_WRITE2(292, 'I',
				atrshmlogline,
			      starttime,
			      endtime,
				 "define function",
	     ...
	     int linehint)
{
	...
	ATRSHMLOG_GETTIME(280,starttime);
	...
	ATRSHMLOG_GETTIME(281,endtime);
		{
		  if (wp[0] && wp[1])
		    {
		      int len = strlen(wp[0]);
		      ATRSHMLOG_WRITE2(281, 'I',
			      linehint,
			      starttime,
			      endtime,
				wp[0],
...
iosetup(struct ioword *iop, struct tbl *tp, int linehint)
{
	...
	ATRSHMLOG_GETTIME(260,starttime);
	...
	case IODUP: {
		...
		if (ksh_isdash(cp)) {
			...
			ATRSHMLOG_GETTIME(261,endtime);
			{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOG_WRITE(261, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	  ...
	  ATRSHMLOG_WRITE(261, 'I',
		...
		if (u == (int)iop->unit) {
			/* "dup from" == "dup to" */
		  ATRSHMLOG_GETTIME(268,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOG_WRITE(268, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
 ...
 clobber_refused:
	...
	if (u < 0) {
		...
		ATRSHMLOG_GETTIME(262,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = 		      strlen(iotmp.ioname);
	  ATRSHMLOG_WRITE(262, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
	...
	if (do_close)
		...
		if (ksh_dup2(u, iop->unit, true) < 0) {
			...
			if (iotype != IODUP)
				close(u);
		ATRSHMLOG_GETTIME(264,endtime);
		{
	  if (iotmp.ioname)
	    {
	      int len = strlen(iotmp.ioname);
	  ATRSHMLOG_WRITE(264, 'I',
		      linehint,
			      starttime,
			      endtime,
		      iotmp.ioname,
