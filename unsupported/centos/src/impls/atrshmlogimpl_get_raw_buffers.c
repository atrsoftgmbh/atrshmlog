 ...
 */
tbuff_t* atrshmlog_il_get_raw_buffers(const int i_buffer_count,
					     const int i_buffer_size)
{
  ...
  if(atrshmlog_init_buffers_in_advance)
    n = calloc(1, i_buffer_count  * (sizeof(tbuff_t) + i_buffer_size + save));
  else
    n = malloc(i_buffer_count  * (sizeof(tbuff_t) + i_buffer_size + save));
