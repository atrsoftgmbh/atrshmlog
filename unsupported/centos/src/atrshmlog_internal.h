...
#define ATRSHMLOG_GETTHREADID ((long)GetCurrentThreadId())
#endif 
...
extern tbuff_t atrshmlog_buffers_prealloced[];
...
extern int atrshmlog_statistics_on;
...
extern tbuff_t* atrshmlog_il_get_raw_buffers(const int i_buffer_count,
					    const int i_buffer_size);
extern int atrshmlog_il_connect_buffers_list(tbuff_t* const raw,
					     const int i_buffer_count,
					     char* i_chunkbuffer,
					     const int i_buffer_size);
extern void atrshmlog_free(tbuff_t* restrict t);
extern tbuff_t* atrshmlog_alloc(void);
extern void atrshmlog_dispatch_buffer(tbuff_t* restrict i_atrshmlog_targetbuffer);
extern tbuff_t *atrshmlog_acquire_buffer(const g_tl_t* restrict i_g);
extern int atrshmlog_init_thread_local (g_tl_t* restrict i_g);
extern void atrshmlog_init_events(const int i_use_file);
extern int atrshmlog_transfer_mem_to_shm(const tbuff_t* restrict i_mem,
...
extern int atrshmlog_init_in_write(g_tl_t* g);
...
#define ATRSHMLOGSTAT(__e) \
  (atrshmlog_statistics_on ? (++atrshmlog_counter[(__e)]) : 0)
