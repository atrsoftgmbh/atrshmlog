...
/** \file atrshmlogreaderc.c
 * \brief We are the concurrent reader.
 *
 * WE read with multiple threads and write buffers.
 */
...
static int f_list_buffer_fetch_wait = ATRSHMLOG_INIT_BUFFER_SLAVE_WAIT;
...
static int f_list_buffer_write_wait = ATRSHMLOG_INIT_BUFFER_SLAVE_WAIT;
...
atomic_int filenumber = ATOMIC_VAR_INIT(1);
		      ...
		      atrshmlog_int32_t i_version)
{
  ...
  //head data is about 118 byte
  // offset is 512, so we have a filler gab of 394 byte
  ...
  if (lcount > 0 && lcount <=  maxbuffersize)
    {
  nb->i_buffer_index = i_buffer_index;
  fn = filenumber++;
  nb->i_filenumber = fn;
  ...
  memcpy(obp,  (void*)&i_buffer_index, sizeof(atrshmlog_int32_t));
  obp += sizeof(atrshmlog_int32_t);
...
static atrshmlog_thread_ret_t f_list_buffer_fetch_proc(void* i_arg)
{
  ...
  while (f_list_buffer_fetch_run)
      ...
      if (tbuff)
	{
	  ...
	  do {
	    for (int index = shm_limit - 1 ; gotone == 0 && index >= 0; index--)
	      {
		int read_ret = read_and_transfer(tbuff,
					       index,
    version);
...
static atrshmlog_thread_ret_t f_list_buffer_write_proc(void* i_arg)
{
  ...
  while (f_list_buffer_write_run)
      ...
      if (tbuff)
	{
	  ...
	  snprintf(target, 256, "%s/%d/atrshmlog_p%%ld_t%%ld_f%d.bin", basepath, dirnumber, filenumber);
