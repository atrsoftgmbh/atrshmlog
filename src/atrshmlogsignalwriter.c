...
/** \file atrshmlogsignalwriter.c
 * \brief We are a communication helper to sent to a reader.
 */
...
/**
 * attach and sent a signal
 * this is for the writes slot.
 *
...
int main (int argc, char*argv[])
{
  ...
  printf("shm log attach and set writer flag and pid.\n");
  printf("logsystem version is %d.\n", ATRSHMLOGVERSION);
  ...
  {
    {
      ...
      atrshmlog_pid_t pid_was_before = a->writerpid;

      int flag_was_before = a->writerflag;
      ...
      if (ATRSHMLOG_GET_AREA_VERSION(a) != ATRSHMLOGVERSION)
    {
      ...
      a->writerflag = (int)lflag;

      a->writerpid = (atrshmlog_pid_t)lpid;
