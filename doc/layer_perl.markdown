Language layer : perl   {#language_layer_perl}
==========================

The perl layer makes the use of the module possible for the perl
development.
...
Atrshmlog.i
-----------
The bridge was made with swig. I first tried XS but had several problems,
so I gave SWIG a try and it did the job.

First you have to learn something about it here. It is not the prefered
way after all.

The module is from now on the perl module - so no confusion to the C module.

The perl module is mostly an one to one expression of the C interface.
...
The interface header from C module and inplace the extern definitions for
...
the perl module file and the wrapper C file.
...
This is for the use from perl.

###Basic functions

The basic functions are implemented as simple functions of the perl module.
...
###start the test in atrshmlogtest.pl
...
Copy the  c library and the two headers to the src directory.
     ...
     $ cd BASEDIR/perl ; bin/getfrommain.sh

Switch to your perl location.

     $ cd src
     
Set the needed perl home with the

     $ . dot.perl.sh


If you use a different perl version you have at least to adjust
